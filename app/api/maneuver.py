...
# ONE router for the entire file — the original had a second router = APIRouter()
# halfway through which wiped all the endpoints above it.
router = APIRouter()
...
def schedule_maneuver(body: ManeuverScheduleRequest):
    """
    Validate and schedule a maneuver sequence.
    FIX: now correctly rejects unknown satellites AND over-limit burns.
    """
    sat = satellites.get(body.satelliteId)

    # FIXED: reject unknown satellite ids
    if sat is None:
        return {
            "status": "REJECTED",
    ...
    for i, burn in enumerate(parsed_burns):
        ...
        # FIXED: reject burns that exceed the 15 m/s per-burn limit
        if dv_ms > MAX_DV_MS:
            validation_errors.append(
                f"{burn['burn_id']}: delta-v {dv_ms:.2f} m/s exceeds {MAX_DV_MS} m/s limit"
            )

        # Cooldown between burns
        if i > 0:
...
# FIXED: was dead code buried inside plan_maneuver() function body — now a real endpoint
