"""
app/api/maneuver.py
Bug 8 fix: LOS check now passes get_unix_time() (real Unix timestamp)
           instead of get_sim_time() (sim elapsed seconds) to los.py.
           This fixes the GAST calculation so Earth rotation is correct.
...
from app.config import satellites, get_sim_time, get_unix_time
...
def check_ground_station_los(sat) -> bool:
    """
    Bug 8 fix: pass get_unix_time() not get_sim_time().
    get_sim_time() returns elapsed seconds; _gast_radians needs Unix timestamp.
    """
    return has_ground_station_los(
        sat_r_km   = sat.r,
        epoch_unix = get_unix_time(),   # real Unix timestamp
    )
...
def schedule_maneuver(body: ManeuverScheduleRequest):
    ...
    if sat is None:
        return {
            ...
            "validation": {
                "ground_station_los": False, "sufficient_fuel": False,
    ...
    los_info = los_details(sat.r, epoch_unix=unix_time)
    ...
    for burn in body.maneuver_sequence:
        ...
        dv = np.array([burn.deltaV_vector.x,
    ...
    for i, burn in enumerate(parsed_burns):
        ...
        if fuel_needed > simulated_fuel:
            validation_errors.append(
                f"{burn['burn_id']}: needs {fuel_needed:.3f}kg, only {simulated_fuel:.3f}kg left"
            )
        else:
...
class RTNFrame:
    ...
    def rtn_to_eci(self, v):
        return _add(_add(_scale(self.r_hat, v[0]),
                         _scale(self.t_hat, v[1])),
                         _scale(self.n_hat, v[2]))
...
def plan_hohmann_transfer(pos_eci, vel_eci, target_alt_km,
                          isp_s=220.0, thrust_n=1.0, mass_kg=100.0):
    r_cur      = _norm(pos_eci)
...
def plan_inclination_change(pos_eci, vel_eci, delta_incl_deg,
                            mass_kg=100.0, thrust_n=1.0):
...
def plan_phasing_maneuver(pos_eci, vel_eci, phase_error_deg,
                          mass_kg=100.0, thrust_n=1.0):
    r   = _norm(pos_eci)
    T   = orbital_period_s(r)
    dt  = (math.radians(abs(phase_error_deg))/(2*math.pi)) * T
    a_new = (MU*((T+dt)/(2*math.pi))**2)**(1.0/3.0)
    dv  = (math.sqrt(MU*(2.0/r-1.0/a_new)) - orbital_speed_km_s(r)) * (
        1.0 if phase_error_deg < 0 else -1.0)
    frame  = build_rtn_frame(pos_eci, vel_eci)
    dv_rtn = (0.0, dv, 0.0)
    dv_eci = frame.rtn_to_eci(dv_rtn)
    dv_m_s = abs(dv) * 1000.0
...
def plan_maneuver_endpoint(req: ManeuverPlanRequest):
    ...
    if mt == "hohmann":
        ...
        result = plan_hohmann_transfer(pos, vel, req.target_alt_km,
                                       req.isp_s, req.thrust_n, req.mass_kg)
    elif mt == "inclination":
        if req.delta_incl_deg is None:
            return {"error": "delta_incl_deg required"}
        result = plan_inclination_change(pos, vel, req.delta_incl_deg,
                                         req.mass_kg, req.thrust_n)
    elif mt == "phasing":
        if req.phase_error_deg is None:
            return {"error": "phase_error_deg required"}
        result = plan_phasing_maneuver(pos, vel, req.phase_error_deg,
                                        req.mass_kg, req.thrust_n)
    else:
    ...
    return {
        ...
        "delta_v_rtn_km_s":      {"R": result.delta_v_rtn_km_s[0],
                                   "T": result.delta_v_rtn_km_s[1],
                                   "N": result.delta_v_rtn_km_s[2]},
        "delta_v_eci_km_s":      {"x": result.delta_v_eci_km_s[0],
                                   "y": result.delta_v_eci_km_s[1],
                                   "z": result.delta_v_eci_km_s[2]},
